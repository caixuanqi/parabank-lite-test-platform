...
![cases](https://img.shields.io/badge/cases-51-success)
...
写 51 条用例，再配一个能看到套件、用例、执行记录、失败归因和手工验证台的测试平台。
...
| 自动化用例 | 51 条 pytest + Selenium 用例，按登录 13 / 注册 22 / 转账 16 分三个套件 | `pytest_suites/` |
| 测试平台 | 展示套件与用例、触发执行、看执行记录、失败归因、手工验证台 | `app.py`、`static/` |
| 单一事实来源 | 业务规则、等价类划分、用例表、预置数据 | `testcase_spec.py` |

核心设计是**单一事实来源**：`testcase_spec.py` 里写着 51 条用例的数据与预期结果，
被测系统的校验逻辑、平台库里的用例目录、pytest 套件、以及 `docs/` 下的用例文档
全部由它派生。用例表一变，`check_consistency.py` 会把不一致的地方全部指出来。

## 架构

**① 单一事实来源与三层结构**：`testcase_spec.py` 是唯一来源（业务规则 20 条 · 等价类 26 个 ·
用例表 51 条 · 预置演示数据），被测系统的校验逻辑、平台的用例目录、pytest 套件与文档全部由它
...
├── testcase_spec.py            业务规则 / 等价类 / 51 条用例 / 预置数据 —— 单一事实来源
...
├── pytest_suites/
│   ├── login_suite/            登录 13 条
│   ├── register_suite/         注册 22 条（逐条重置演示数据，保证"用户名未注册"）
│   ├── transfer_suite/         转账 16 条
│   └── manual_suite/           手工验证台：跑一次真实操作，只记录系统实际返回
├── docs/
│   ├── 测试用例设计.md          业务规则 + 等价类 + 用例表（51 条）+ 文案对照 + 缺陷清单
...
| 账号 | 密码 | 账户 | 余额 | 用途 |
|------|------|------|------|------|
| admin_01 | Admin@123 | 10001 / 10002 / 10003 | 100000.00 / 0.00 / 100.00 | 登录与转账模块的合规账号（用例表指定） |
| admin | admin123 | — | — | 文档"初始账号"，保留 |
| user_2026 | Pass123 | 20002 | 0.00 | 等价类划分表中列出的有效数据 |
...
- 三个场景：登录 / 注册 / 转账；留空的字段用预置值（登录 = `admin_01/Admin@123`）
...
实测耗时（本机无头 Edge）：登录套件 14s、注册套件 40s、转账套件 26s，全套 51 条约 80 秒。
...
用例表严格按《ParaBank Lite 业务规则与测试用例设计（最新版）》与三张模块用例表落地，
共 **51 条**：

| 模块 | 规则数 | 等价类数 | 用例数 | P0 | P1 | P2 | 套件 |
|------|--------|----------|--------|----|----|----|------|
| 登录 | 7 | 8 | 13 | 3 | 3 | 7 | `login_suite` |
| 注册 | 6 | 9 | 22 | 8 | 13 | 1 | `register_suite` |
| 转账 | 7 | 9 | 16 | 6 | 8 | 2 | `transfer_suite` |
| **合计** | **20** | **26** | **51** | **17** | **24** | **10** | |

设计文档里同时记录了**用例表自身需要澄清的地方与落地口径**：

- 登录用户名长度按最新版文档为 **6~20 位**（旧版文档的 3~20 已作废）
- LOGIN_011 文档数据 `Admin1` 是 6 位、触发不了"密码过短"，落地用 5 位 `Admin`
- REG_010 文档用 `admin`（5 位）触发"用户名已存在"，过不了 6~20 长度校验，
  落地改用预置账号 `admin_01`
- LOGIN_009 / REG_008 的含汉字用户名不足 6 位，落地补足位数以命中字符集校验
- 转账用例编号沿用文档的 `TRAN_01 ~ TRAN_16`（登录/注册为三位序号）
- 文档文案中的排版空格（如"金额必须大于 0"）落地时去掉
...
| BUG-02 金额上限边界写成"≥50000 即超限" | 2.3(2) | TRAN_05 | `ASSERT_` |
| BUG-03 漏做"仅允许一个小数点"校验 | 2.3(4) | TRAN_11 | `ASSERT_` |

预期结果：**51 条用例 47 通过 / 4 失败**，失败集合必须正好等于上表——
多红一条或少红一条，`check_consistency.py` 都会报错。

关掉注入缺陷（恢复"完全符合文档"的系统，51 条应当全绿）：把 `parabank_lite.py` 里的
...
| 文档 | 内容 |
|------|------|
| [docs/测试用例设计.md](docs/测试用例设计.md) | 业务规则、等价类划分、51 条用例表、提示文案对照、缺陷清单（由 `gen_case_doc.py` 生成） |
| [docs/测试规则文档.md](docs/测试规则文档.md) | 用例编号、优先级、结果判定与失败归因、套件规范、触发规则、一致性维护流程 |
| [docs/数据库设计文档.md](docs/数据库设计文档.md) | 平台库六张表结构、索引、数据来源与预置数据 |

## 常见问题

**Q：为什么登录/转账用的是 `admin_01`，而不是 `admin`？**
最新版用例表把登录用户名长度定为 6~20 位，`admin` 只有 5 位、过不了长度校验，
因此预置了 8 位的 `admin_01` 作为登录与转账的合规账号；`admin` 仍然保留（文档"初始账号"）。

**Q：注册套件为什么逐条重置演示数据？**
用例表里多条"注册成功"用例复用同一个用户名（如 `alice01`），若不重置，第二条起就会
撞上"用户名已存在"。逐条重置让每条用例都满足"用户名未注册"这一隐含前置条件。
...
预置账号（`admin_01/Admin@123`、`admin/admin123`、`user_2026/Pass123`）是自建演示系统的
固定测试账号，仅用于本机演示与自动化测试。请勿将本项目的 Flask 开发服务器与演示账号
用于生产环境。
