...
```
                  ┌──────────────────────── testcase_spec.py ────────────────────────┐
                  │  业务规则 21 条 · 等价类 24 个 · 用例表 39 条 · 预置演示数据        │
                  └───────┬─────────────────┬─────────────────────┬──────────────────┘
                          │                 │                     │
                生成 ↓    │        生成 ↓   │            校验 ↕    │   生成 ↓
        ┌─────────────────┴───┐  ┌──────────┴────────┐  ┌─────────┴──────────┐
        │ 被测系统 parabank_lite │  │ 测试平台用例目录   │  │ pytest 套件 + 文档 │
        │ 校验规则 / 提示文案    │  │ app.py seed_data  │  │ 39 条参数化用例     │
        └─────────┬────────────┘  └─────────┬─────────┘  └─────────┬──────────┘
                  │  真实浏览器操作          │ 套件执行 / 手工验证  │
                  └────────────► Selenium 无头浏览器 ◄─────────────┘
                                        │
                     pytest-json-report + 观察文件 ──► test_results
                                        │
                              平台页面：用例 / 执行记录 / 结果明细
```
...
```
平台 → 起子进程 pytest pytest_suites/<suite>/...（或 manual_suite 的某个场景）
     → pytest 用 Selenium 驱动无头 Edge 打开 http://127.0.0.1:5000/parabank
     → 断言（用例套件）或记录观察结果（手工验证台）
     → pytest-json-report 产出 JSON，平台解析后写进 test_results
     → 页面轮询显示结果与失败归因
```
